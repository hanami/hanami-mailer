...
RSpec.describe Hanami::Mailer::DSL::CleverProc do
  describe ".from_name" do
    it "returns CleverProc when given a proc" do
      proc = -> { "result" }
      context = Object.new

      clever_proc = described_class.from_name(proc, :unused_name, context)

      expect(clever_proc).to be_a(described_class)
      expect(clever_proc.proc).to eq(proc)
    end

    it "returns CleverProc when context responds to method name" do
      ...
      clever_proc = described_class.from_name(nil, :my_method, context)

      expect(clever_proc).to be_a(described_class)
      expect(clever_proc.proc).to be_a(Method)
    end
    ...
    it "finds private methods on context" do
      ...
      clever_proc = described_class.from_name(nil, :private_method, context)
    ...
    it "returns nil when no proc and context doesn't respond to name" do
      ...
      clever_proc = described_class.from_name(nil, :nonexistent, context)

      expect(clever_proc).to be_nil
    end
    ...
    it "prefers proc over method lookup" do
      ...
      clever_proc = described_class.from_name(proc, :my_method, context)

      expect(clever_proc.call({})).to eq("from proc")
    end
  end
  ...
  describe "#call" do
    describe "with no parameters" do
      it "calls proc with no arguments" do
        proc = -> { "simple result" }
        context = Object.new
        clever_proc = described_class.new(proc, context: context)

        result = clever_proc.call({})

        expect(result).to eq("simple result")
      end

      it "returns nil when proc is nil" do
        clever_proc = described_class.new(nil, context: Object.new)

        result = clever_proc.call({foo: "bar"})
    ...
    describe "with keyword parameters" do
      it "extracts required keyword arguments from input" do
        ...
        result = clever_proc.call({name: "Alice", extra: "ignored"})
      ...
      it "extracts optional keyword arguments from input" do
        ...
        result = clever_proc.call({greeting: "Hello"})
      ...
      it "extracts multiple keyword arguments" do
        ...
        result = clever_proc.call({first: "John", last: "Doe", title: "Dr."})
    ...
    describe "with keyrest (**kwargs)" do
      it "passes all input as kwargs" do
        ...
        result = clever_proc.call({a: 1, b: 2, c: 3})
      ...
      it "extracts named keywords while passing rest to keyrest" do
        ...
        result = clever_proc.call({name: "Alice", age: 30, city: "NYC"})
    ...
    describe "with positional parameters (dependencies)" do
      it "passes positional arguments through" do
        ...
        result = clever_proc.call({}, "first", "second")
      ...
      it "combines positional args with keyword extraction" do
        ...
        result = clever_proc.call({value: 10}, 3)
    ...
    describe "context binding" do
      ...
      it "executes proc with keywords in context" do
        ...
        result = clever_proc.call({message: "hello"})
      ...
      it "calls Method directly without instance_exec" do
        ...
        clever_proc = described_class.new(method, context: context)

        result = clever_proc.call({factor: 3})
  ...
  describe "#callable?" do
    it "returns true when proc is present" do
      clever_proc = described_class.new(-> {}, context: Object.new)

      expect(clever_proc.callable?).to be true
    end

    it "returns false when proc is nil" do
      clever_proc = described_class.new(nil, context: Object.new)

      expect(clever_proc.callable?).to be false
    end
  end
  ...
  describe "#dependency_names" do
    it "returns empty array when no proc" do
      ...
      expect(clever_proc.dependency_names).to eq([])
    end
  ...
  describe "#keyword_names" do
    it "returns empty array when no proc" do
      ...
      expect(clever_proc.keyword_names).to eq([])
    end
